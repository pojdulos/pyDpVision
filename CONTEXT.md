...
- Dla meshy pojawia sie tez drugi backend eksperymentalny `projected_intersection_list`, zostawiony obok `analytic_bvh`. Projektuje trojkaty na detektor, buduje skompresowany stack przeciec per piksel (`pixel_offsets` + plaskie tablice probek), a potem moze liczyc `solid/shell` przez sortowanie i parowanie wartosci `t` juz w przestrzeni obrazu.
- W trybie `shell` backend `projected_intersection_list` scala teraz prawie-identyczne trafienia w jeden klaster i usrednia ich gain katowy, zeby wspolne krawedzie oraz wierzcholki sasiednich trojkatow nie byly liczone jako dodatkowe warstwy materialu.
- Budowa stacku `projected_intersection_list` cache'uje teraz trafienia trojkata z pierwszego przejscia i uzywa ich ponownie przy skladaniu plaskich tablic probek. Liczniki trafien sa aktualizowane rzadko tylko dla pikseli faktycznie trafionych przez dany trojkat, a zapis do `sample_t/sample_triangle_index/sample_shell_gain` jest grupowany blokami per piksel zamiast pojedyncza petla po kazdym trafieniu.
- W trybie `solid` samotne nieparzyste trafienie nie jest juz zamieniane na zastępczy wkład `shell`. Taki fallback dawal lokalne jasne artefakty dla nieszczelnych albo niejednoznacznych fragmentow siatki; teraz takie piksele sa po prostu ignorowane jako niepewne.
- `MeshQualityAnalyzer` ma teraz tez lekki raport topologii pod RTG: liczy m.in. `degenerate faces`, `duplicate faces`, `boundary edges`, `nonmanifold edges` i lokalne konflikty orientacji na wspolnych krawedziach. W `dp_testy.py` sa helpery `report_mesh_xray_topology(mesh)` oraz `report_selected_mesh_xray_topology()`.
- W `dp_testy.py` jest tez prosty cleanup pod RTG: `clean_mesh_for_xray(mesh)` oraz `clean_selected_mesh_for_xray()`. Ten krok scala zdublowane wierzcholki po kwantyzacji pozycji, usuwa zdegenerowane trojkaty powstale po scaleniu i wycina duplicate faces niezaleznie od windingu. Ma tez opcjonalne flagi `drop_nonmanifold_faces` i `drop_boundary_faces`, ktore odrzucaja cale trojkaty dotykajace odpowiednio krawedzi non-manifold albo boundary, po czym wypisywany jest raport topologii przed/po.
- W `dp_testy.py` jest tez helper `run_virtual_xray_headless(virtual_xray, save_png_path=None)` oraz wygodny wrapper `run_selected_virtual_xray_headless()`. Uruchamiaja `VirtualXRay` bez wstawiania obrazu do workspace i bez sciezki GUI `Run Simulation`, co sluzy do rozroznienia: crash w samym backendzie projekcji kontra crash podczas tworzenia `Image`, odswiezania workspace lub repaintu GL.
- Sciezka tworzenia nowego obrazu projekcji w `PropVirtualXRay` nie uzywa juz `Image(image=qimage)` bezposrednio. Tworzy najpierw pusty `Image()`, a potem dopiero wywoluje `setImage(qimage)`, bo konstruktor z przekazywanym `QImage` byl podejrzany jako potencjalne zrodlo crasha.
- `MeshXRaySource` ma teraz tez wbudowany eksport map diagnostycznych dla backendu `projected_intersection_list`. Jesli mesh ma ustawione `xray_debug_export_dir`, to podczas projekcji zapisywane sa obrazy: `projected_raw_hit_count`, `projected_merged_hit_count`, `projected_odd_mask`, `projected_path_length`, a takze mapy kata padania `projected_max_shell_gain` oraz `projected_min_abs_cos`. Opcjonalnie moze byc tez zapisane porownanie do `analytic_bvh` (`analytic_merged_hit_count`, `hit_count_mismatch`, `path_length_abs_diff`). Flaga `xray_debug_compare_analytic` steruje czy liczony jest drozszy wariant porownawczy.
- Mesh moze tez miec teraz atrybut `xray_projected_min_abs_cos` (domyslnie `0.0`). Dla backendu `projected_intersection_list` trafienia z bardzo malym `|n·d|` sa wtedy odrzucane jeszcze przed dalszym sumowaniem. To sluzy jako szybki eksperymentalny filtr na prawie styczne fragmenty powierzchni i jest na razie wystawione tylko jako pole backendowe, bez GUI.
...
- W `docs/main.tex` sekcja `Method` jest dalej doprecyzowywana w stylu artykułowym: opisy modeli maja byc samowystarczalne, tzn. obejmowac nie tylko nazwy trybow z implementacji, ale tez ich sens matematyczny, interpretacje i konsekwencje numeryczne.
- W `docs/main.tex` doprecyzowano juz m.in. semantyke trybow interpolacji, sposob rekonstrukcji bazy detektora z parametrow `center/normal/up`, znaczenie modeli `piecewise_*`, sens okna materialowego oraz to, ze profile jakosci sa gotowymi zestawami parametrow numerycznych, a nie odrebnymi modelami fizycznymi.
