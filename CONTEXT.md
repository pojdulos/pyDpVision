...
- W `dp_testy.py` sa tez pomocnicze scenariusze `create_synthetic_xray_demo_dicoms()` i `demo_synthetic_xray_projection()`, ktore buduja dwa proste syntetyczne zestawy DICOM (czaszka i zuchwa) oraz pokazują podstawowe użycie backendu RTG z eksportem `PNG`, `TIFF` i `DICOM`.
